...
class SendingUnavailableError(PermissionError):
    """Raised because version 0.1.0 cannot send email."""
...
class EmailPlugin:
    ...
    async def send_message(self, draft_id: str) -> None:
        """Refuse sending unconditionally in version 0.1.0."""
        del draft_id
        raise SendingUnavailableError("email sending is not implemented in version 0.1.0")


def register(ctx: Any) -> None:
    """Register the bundled email skill with Hermes.

    Version 0.1.0 deliberately registers no tools, hooks, providers, pollers,
