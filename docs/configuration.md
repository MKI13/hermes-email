...
No configuration option enables attachment content access in v0.32.0. IMAP may normalize bounded attachment metadata for message/thread detail only. Attachment bytes are never returned to Hermes tools, stored as files, opened, executed, or uploaded. Filename and MIME metadata remain untrusted external data.
...
Attachment handling classification is deterministic and requires no trust configuration; automatic open, execute, and content access remain disabled in v0.32.0.
