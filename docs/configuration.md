...
No configuration option enables attachment content access in v0.35.0. IMAP may normalize bounded attachment metadata for message/thread detail only. Attachment bytes are never returned to Hermes tools, stored as files, opened, executed, or uploaded. Filename and MIME metadata remain untrusted external data.
...
Attachment handling classification is deterministic and requires no trust configuration; automatic open, execute, and content access remain disabled in v0.35.0.

### `audit`

`mode` is `disabled` or `sqlite`. SQLite audit is content-minimized, profile-scoped, bounded by retention/event/database limits, and requires an explicit profile owner.
...
## v0.35.0 Proton/local bridge example
...
`own_addresses` is an optional bounded list of operator-owned mailbox addresses. It is required before the Reply-All draft tool becomes available. These addresses are used only to exclude self from original recipients; they grant no authority and do not imply sender trust.
