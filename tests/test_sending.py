...
def prepare(
    configuration: EmailPluginConfig,
...
def test_prepares_deterministic_plain_text_bytes_and_complete_envelope(
    tmp_path: Path,
) -> None:
    ...
    assert first.envelope_recipients == (
        "to@example.invalid",
    ...
    assert b"\r\n" in first.message_bytes
    assert b"\n" not in first.message_bytes.replace(b"\r\n", b"")


def test_disabled_gate_rejects_before_opening_draft_database(tmp_path: Path) -> None:
    configuration = EmailPluginConfig.from_mapping(
        {
            "drafts": {
                "mode": "sqlite",
                "account_namespace": "smtp-account",
...
def test_candidate_is_owned_snapshot_not_changed_by_later_draft_update(
    tmp_path: Path,
) -> None:
    configuration = config()
    drafts = store(tmp_path, configuration)
    ...
    drafts.update_draft(
        ...
        1,
        EmailDraft(
            recipients=(EmailAddress("other@example.invalid"),),
            subject="Changed",
            body_text="Changed",
        ),
        "update-operation-0001",
