...
def test_armed_runtime_is_offline_and_exposes_no_submission_surface(
    ...
    assert "email_review_draft_send" in {tool["name"] for tool in context.tools}
    assert not ({"email_send", "email_send_draft", "email_confirm_send"} & {tool["name"] for tool in context.tools})
