...
from hermes_email.draft_tools import CREATE_DRAFT_TOOL, CREATE_REPLY_DRAFT_TOOL, CREATE_REPLY_ALL_DRAFT_TOOL, GET_DRAFT_TOOL
...
def test_enabled_registration_uses_public_state_path_but_remains_lazy(
    ...
    assert all(tool["check_fn"]() is True for tool in context.tools[:8] if tool["name"] not in {CREATE_REPLY_DRAFT_TOOL, CREATE_REPLY_ALL_DRAFT_TOOL})
    assert next(tool for tool in context.tools[:8] if tool["name"] == CREATE_REPLY_DRAFT_TOOL)["check_fn"]() is False
    assert database.exists() is False
...
def test_read_tool_collision_rolls_back_all_prior_draft_tools(tmp_path: Path) -> None:
    ...
    assert len(context.handles) == 8
    assert all(handle.disposed for handle in context.handles)


def test_skill_failure_rolls_back_all_ten_tools_and_closes_runtime(
    tmp_path: Path,
) -> None:
