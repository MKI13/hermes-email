...
Version `0.10.1` implements only a guarded facade over deterministic mock retrieval; it does not implement production message retrieval or sending. Never commit credentials, tokens, passwords, private keys, real messages, or account-specific configuration. Treat third-party plugins and future provider adapters as trusted code running with the user's permissions.
