...
Use this skill to analyze email content or prepare a draft while preserving the active Hermes agent's identity and policies. Version 0.5.0 provides guidance only; it does not connect to an account or perform mailbox operations.
...
No credentials, provider account, or external email service is required in version 0.5.0. Work only with content the user explicitly provides in the active session.
