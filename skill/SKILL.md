...
Use this skill to analyze email content or prepare a draft while preserving the active Hermes agent's identity and policies. Version 0.8.0 adds only guarded retrieval from the local mock provider; it does not connect to a real account or perform mailbox side effects.
...
No credentials, provider account, or external email service is required in version 0.8.0. Work only with user-supplied content or deterministic messages returned by an explicitly enabled mock provider.
